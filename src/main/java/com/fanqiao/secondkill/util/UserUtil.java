...
public class UserUtil {
	...
	private static void createUser(int count) throws Exception {
		...
		for (int i = 0; i < count; i++) {
			...
			user.setPassword(MD5Util.formPasswordToDatabasePassword(MD5Util.inputPasswordToFormPassword("123456"), "54fafdas6"));
			users.add(user);
		}
		...
		for (int i = 0; i < users.size(); i++) {
			...
			pstmt.setTimestamp(3, new Timestamp(user.getRegistDate().getTime()));
	...
	//模拟登录生成token
		...
		for(int i=0;i<users.size();i++) {
			...
			String params = "mobile="+user.getMobile()+"&password="+MD5Util.inputPasswordToFormPassword("123456");
